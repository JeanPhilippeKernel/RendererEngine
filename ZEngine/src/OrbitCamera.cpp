...
namespace ZEngine::Rendering::Cameras
{
    ...
    void OrbitCamera::SetPosition(const glm::vec3& position)
    {
        ...
        glm::quat quat_around_z   = glm::angleAxis(0.0f, glm::vec3(0.0f, 0.0f, 1.0f));
        glm::quat quat_around_zyx = quat_around_z * quat_around_y * quat_around_x;

        //m_forward  = glm::rotate(glm::normalize(quat_around_zyx), glm::normalize(m_forward));
        m_position = m_target + (m_radius * m_forward);
    ...
    void OrbitCamera::SetPosition(float yaw_degree, float pitch_degree)
    {
        ...
        //m_forward  = glm::rotate(glm::normalize(quat_around_zyx), m_forward);
        m_position = m_target + (m_radius * m_forward);
