...
namespace ZEngine::Rendering::Textures
{
    Ref<Texture2D> Texture2D::Read(std::string_view filename)
    {
        ...
        */
        ...
        void* memory_region;
        ZENGINE_VALIDATE_ASSERT(vkMapMemory(device, staging_buffer.Memory, 0, texture->m_buffer_size, 0, &memory_region) == VK_SUCCESS, "Failed to map the memory")
        std::memcpy(memory_region, image_data_rgba, static_cast<size_t>(texture->m_buffer_size));
        vkUnmapMemory(device, staging_buffer.Memory);

        stbi_image_free(image_data_rgba);

        /* Create VkImage */
        texture->m_image_2d_buffer = CreateRef<Buffers::Image2DBuffer>(width, height, VK_FORMAT_R8G8B8A8_SRGB,
    ...
    Ref<Texture2D> Texture2D::Create(uint32_t width, uint32_t height)
    {
        ...
        void* memory_region;
        ZENGINE_VALIDATE_ASSERT(vkMapMemory(device, staging_buffer.Memory, 0, texture->m_buffer_size, 0, &memory_region) == VK_SUCCESS, "Failed to map the memory")
        std::memcpy(memory_region, image_data, static_cast<size_t>(texture->m_buffer_size));
        vkUnmapMemory(device, staging_buffer.Memory);

        /* Create VkImage */
        texture->m_image_2d_buffer =
