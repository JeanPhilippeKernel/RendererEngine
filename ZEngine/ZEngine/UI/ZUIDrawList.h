...
namespace ZEngine::UI
{
    ...
    // Clip rect stack
    // ---------------------------------------------------------------
    void ZUIDrawListPushClipRect(ZUIDrawList* dl,
    ...
    // Shape primitives
    // ---------------------------------------------------------------
    void ZUIDrawListAddLine(ZUIDrawList* dl,
                             float x0, float y0, float x1, float y1,
                             uint32_t col, float thickness = 1.f);

    // Anti-aliased filled convex polygon from an array of (x,y) pairs.
    void ZUIDrawListAddPolylineFilled(ZUIDrawList* dl,
    ...
    void ZUIDrawListAddRectFilled(ZUIDrawList* dl,
                                   float x0, float y0, float x1, float y1,
                                   uint32_t col,
                                   float rounding = 0.f, uint32_t round_flags = 0xF);

    // Stroked (outline) rect
    void ZUIDrawListAddRect(ZUIDrawList* dl,
                             float x0, float y0, float x1, float y1,
                             uint32_t col, float rounding = 0.f,
                             uint32_t round_flags = 0xF, float thickness = 1.f);

    void ZUIDrawListAddCircleFilled(ZUIDrawList* dl,
                                     float cx, float cy, float r,
                                     uint32_t col, int num_segments = 0);

    void ZUIDrawListAddCircle(ZUIDrawList* dl,
                               float cx, float cy, float r,
                               uint32_t col, int num_segments = 0,
                               float thickness = 1.f);
    ...
    void ZUIDrawListAddImage(ZUIDrawList* dl,
                              uint32_t tex_idx,
                              float x0, float y0, float x1, float y1,
    ...
    inline uint32_t ZUIPackColor(const float c[4])
    {
        ...
        return (uint32_t)clamp01(c[0])
             | ((uint32_t)clamp01(c[1]) << 8)
             | ((uint32_t)clamp01(c[2]) << 16)
             | ((uint32_t)clamp01(c[3]) << 24);
    }
