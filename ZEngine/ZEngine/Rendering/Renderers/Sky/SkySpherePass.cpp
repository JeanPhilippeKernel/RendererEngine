...
namespace ZEngine::Rendering::Renderers
{
    ...
    void SkySpherePass::Compile(Hardwares::VulkanDevicePtr const device, Rendering::Scenes::SceneDataPtr const /*scene*/, RenderPasses::RenderPassBuilder* pass_builder, RenderGraphResourceInspectorPtr /*res_inspector*/, RenderPasses::RenderPass** const output_pass)
    {
        ...
        {
            auto pass_spec = pass_builder->SetPipelineName("SkySphere-Pipeline")
                                 .SetInputBindingCount(0)
                                 .EnablePipelineDepthTest(true)
                                 .EnablePipelineDepthWrite(false)
                                 .PipelineDepthCompareOp(static_cast<uint32_t>(VK_COMPARE_OP_LESS_OR_EQUAL))
                                 .SetCullMode(static_cast<uint32_t>(VK_CULL_MODE_NONE))
                                 .UseShader("sky_sphere")
                                 .Detach();
