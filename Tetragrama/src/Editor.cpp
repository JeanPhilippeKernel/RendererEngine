...
namespace Tetragrama {
	...
	void Editor::Initialize() {
		...
		m_rendering_layer.reset(new Layers::ExampleLayer(shared_from_this()));
	...
	void Editor::OnUIComponentRaised(UIComponentEvent& e) {
		const auto rendering_layer_ptr = dynamic_cast<Layers::ExampleLayer*>(m_rendering_layer.get());
		...
		if (rendering_layer_ptr) {
			event_dispatcher.ForwardTo<Components::Event::SceneViewportResizedEvent>(std::bind(&Layers::ExampleLayer::OnUIComponentRaised, rendering_layer_ptr, std::placeholders::_1));
		}
