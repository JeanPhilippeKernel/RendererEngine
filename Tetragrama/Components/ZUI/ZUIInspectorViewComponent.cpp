...
namespace Tetragrama::Components
{
    ...
    static bool XYZDragRow(ZUIContext* ctx, const char* row_key, const char* label, float* x, float* y, float* z, float speed)
    {
        ZUIBeginRow(ctx, row_key, ZFill(), ZSPx(ctx, kRowH));
        ZUIBox* lbl       = ZUIPushBox(ctx, label, (uint32_t) ZEngine::Helpers::secure_strlen(label), ZUI_DrawText);
        lbl->Size[0]      = ZPx(kLabelW);
    ...
    void ZUIInspectorViewComponent::Initialize(Tetragrama::Layers::ZUILayer* parent, cstring name, bool visibility)
    {
        ParentLayer = parent;
        Name        = name;
        Visible     = visibility;
    ...
    void ZUIInspectorViewComponent::BuildUI(ZUIContext* ctx)
    {
        ...
        if (tc)
        {
            ...
            if (m_transform_open)
            {
                ...
                XYZDragRow(ctx, "##loc", "Location", &tc->Position.x, &tc->Position.y, &tc->Position.z, 0.05f);

                ZUITableNextRow(ctx);
                ZUITableSetColumn(ctx, 0);
                ZUITableSetColumn(ctx, 1);
                {
                    float deg[3] = {tc->Rotation.x * 57.2957f, tc->Rotation.y * 57.2957f, tc->Rotation.z * 57.2957f};
                ...
                ZUITableSetColumn(ctx, 0);
                ZUITableSetColumn(ctx, 1);
                XYZDragRow(ctx, "##scl", "Scale", &tc->Scale.x, &tc->Scale.y, &tc->Scale.z, 0.01f);
