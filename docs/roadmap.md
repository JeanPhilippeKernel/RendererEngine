...
| **Memory** | Named, independently reserved CPU owners validated against an 8 GiB configured-capacity limit (Default 7,604 MiB; Editor 7,868 MiB; Server 6,324 MiB), sub-arenas, scratch pairs, `PaddedAtomic<T>`, and `MemoryBudgetConfig`; `Array<T>` is move-only and `HashMap`/`UnorderedHashMap` were rewritten (PR [#615](https://github.com/JeanPhilippeKernel/RendererEngine/pull/615)). See `ZEngine/docs/memory-budget.md` for the current profile. |
