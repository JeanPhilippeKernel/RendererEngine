...
| **Memory** | 8 GiB root arena with configured CPU profiles (Default 7,060 MiB; Editor 6,932 MiB; Server 5,780 MiB), sub-arenas, scratch pairs, `PaddedAtomic<T>`, and `MemoryBudgetConfig`; `Array<T>` is move-only and `HashMap`/`UnorderedHashMap` were rewritten (PR [#615](https://github.com/JeanPhilippeKernel/RendererEngine/pull/615)). See `ZEngine/docs/memory-budget.md` for the current profile. |
