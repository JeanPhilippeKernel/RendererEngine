...
layout (location = 5) in float	a_texture_tiling_factor;
layout (location = 6) in vec4	a_texture_tint_color;
...
//this part is for the future
//uniform float texture_tiling_factor[32];
//uniform float texture_tint_color[32];
...
void main()
{
	output_color = texture(
		uniform_texture_slot[int(texture_slot_id)], 
		texture_coord * texture_tiling_factor
	) * texture_tint_color;
}
