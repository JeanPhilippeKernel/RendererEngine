...
namespace Z_Engine::Rendering::Meshes {
	...
	struct IMesh
	{
		...
		IMesh(const char * shader_name,
			const std::vector<T>& data, 
			const std::vector<K>& index,
			std::initializer_list<Z_Engine::Rendering::Buffers::Layout::ElementLayout<T>> elements_list) 
			:
			m_shader_manager(new Z_Engine::Managers::ShaderManager()),
			m_shader(m_shader_manager->Obtains(shader_name)),
			m_data(data), m_index(index), m_internal_layout(elements_list),
	...
	protected:
	   Z_Engine::Ref<Z_Engine::Managers::ShaderManager>								m_shader_manager;
